...
def nn_layer(input_tensor,
    ...
    weights, bias = params
    return params, tf.sigmoid(tf.matmul(input_tensor, weights) + bias)
...
def lstm_layer(state_tensor, lstm_output, input_tensor, name, params=None):
    ...
    state_row_dim, state_col_dim = state_tensor.get_shape()
...
def graph_layer(input_tensor, prev_lstm_state, prev_lstm_output,
    ...
    input_rows = input_tensor.get_shape()[0].value
...
def unroll_lstm(depth, init_lstm_state, init_lstm_output, params):
    ...
    for i in xrange(depth):
        ...
        losses.append(
            tf.reduce_sum(
                crossentropy_loss(
                    tf.reshape(lstm_output, (output.get_shape()[1].value,)),
